...
void syscall(void)
{
  ...
  num = curproc->tf->eax; // Get system call number from %eax
  ...
  if(num == 7){
    ...
    // cprintf("new shell created with number of shells as :- %d\n",current_sh);
  }
  if (num > 0 && num < NELEM(syscalls) && syscalls[num]){
    // cprintf("current pid = %d and its parent pid = %d\n",curproc->pid, curproc->parent->pid);
    // if(curproc->parent->pid == 2 && num == 7){
